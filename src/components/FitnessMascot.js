...
// Place mascot_male.png and mascot_female.png in assets/ folder
// These should be front-view 3D anatomy renders (no face/mannequin style)
// Recommended size: 400×800 px, PNG with transparent background
let MALE_IMG   = null;
let FEMALE_IMG = null;
try { MALE_IMG   = require('../../assets/mascot_male.png');   } catch {}
try { FEMALE_IMG = require('../../assets/mascot_female.png'); } catch {}
...
function FallbackMascot({ gender = 'male' }) {
  ...
  const W  = 170;
  const cx = W / 2;
  const sw = m ? 136 : 108; const cw = m ? 82 : 72; const ww = m ? 62 : 50;
  const hw = m ? 74 : 88;   const aw = m ? 26 : 22; const fw = m ? 22 : 18;
  const tw = m ? 35 : 33;   const clw = m ? 25 : 23;
...
export default function FitnessMascot({ gender = 'male', style }) {
  const imgSrc = gender === 'female' ? FEMALE_IMG : MALE_IMG;

  if (imgSrc) {
    return (
      <View style={[{ width: 200, height: 360, alignItems: 'center' }, style]}>
        <Image
          source={imgSrc}
  ...
  // Fallback View-based mascot when images not available
  return <FallbackMascot gender={gender} />;
}
